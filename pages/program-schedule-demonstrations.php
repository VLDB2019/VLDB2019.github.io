<section class="program-schedule-demonstrations">
    <h1>VLDB 2019: Demonstrations</h1>

    <p>
        <strong class="accepted-title">GALO: Guided Automated Learning for re-Optimization</strong>
        <span class="accepted-authors">
            <span class="author">Guilherme Damasio</span> (University of Ontario Institute of Technology),
            <span class="author">Spencer Bryson</span> (University of Ontario Institute of Technology),
            <span class="author">Vincent Corvinelli</span> (IBM),
            <span class="author">Parke Godfrey</span> (York University),
            <span class="author">Piotr Mierzejewski</span> (IBM),
            <span class="author">Jaroslaw Szlichta</span> (University of Ontario Institute of Technology and IBM), and
            <span class="author">Calisto Zuzarte</span> (IBM)
        </span>
    </p>
    <p>
        <strong class="accepted-title">Synergistic Graph and SQL Analytics Inside IBM Db2</strong>
        <span class="accepted-authors">
            <span class="author">Yuanyuan Tian</span> (IBM Almaden),
            <span class="author">Sui Jun Tong</span> (IBM Research),
            <span class="author">Hamid Pirahesh</span> (IBM Research), and
            <span class="author">Wen Sun</span> (IBM)
        </span>
    </p>
    <p>
        <strong class="accepted-title">Cleanits: A Data Cleaning System for Industrial Time Series</strong>
        <span class="accepted-authors">
            <span class="author">Xiaoou Ding</span> (Harbin Institute of Technology),
            <span class="author">Hongzhi Wang</span> (Harbin Institute of Technology),
            <span class="author">Jiaxuan Su</span> (Harbin Institute of Technology),
            <span class="author">Zijue Li</span> (Harbin Institute of Technology),
            <span class="author">Jianzhong Li</span> (Harbin Institute of Technology), and
            <span class="author">Hong Gao</span> (Harbin Institute of Technology)
        </span>
    </p>
    <p>
        <strong class="accepted-title">ITAA: An Intelligent Trajectory-driven Outdoor Advertising Deployment Assistant</strong>
        <span class="accepted-authors">
            <span class="author">Yipeng Zhang</span> (RMIT University),
            <span class="author">Zhifeng Bao</span> (RMIT University),
            <span class="author">Songsong Mo</span> (Wuhan University),
            <span class="author">Yuchen Li</span> (Singapore Management University), and
            <span class="author">Yanghao Zhou</span> (NUAA)
        </span>
    </p>
    <p>
        <strong class="accepted-title">SystemER: A Human-in-the-loop System for Explainable Entity Resolution</strong>
        <span class="accepted-authors">
            <span class="author">Kun Qian</span> (IBM Research - Almaden),
            <span class="author">Lucian Popa</span> (IBM Almaden Research Center), and
            <span class="author">Prithviraj Sen</span> (IBM Almaden Research Center)
        </span>
    </p>
    <p>
        <strong class="accepted-title">Buckle: Evaluating Fact Checking Algorithms Built on Knowledge Bases</strong>
        <span class="accepted-authors">
            <span class="author">Viet-Phi Huynh</span> (EURECOM) and
            <span class="author">Paolo Papotti</span> (EURECOM)
        </span>
    </p>
    <p>
        <strong class="accepted-title">A Query System for Efficiently Investigating Complex Attack Behaviors for Enterprise Security</strong>
        <span class="accepted-authors">
            <span class="author">Peng Gao</span> (University of California, Berkeley),
            <span class="author">Xusheng Xiao</span> (Case Western Reserve University),
            <span class="author">Zhichun Li</span> (NEC Labs),
            <span class="author">Kangkook Jee</span> (NEC Labs),
            <span class="author">Fengyuan Xu</span> (Nanjing University),
            <span class="author">Sanjeev Kulkarni</span> (Princeton University), and
            <span class="author">Prateek Mittal</span> (Princeton University)
        </span>
    </p>
    <p>
        <strong class="accepted-title">CAPE: Explaining Outliers by Counterbalancing</strong>
        <span class="accepted-authors">
            <span class="author">Zhengjie Miao</span> (Duke University),
            <span class="author">Qitian Zeng</span> (Illinois Institute of Technology),
            <span class="author">Chenjie Li</span> (Illinois Institute of Technology),
            <span class="author">Boris Glavic</span> (Illinois Institute of Technology),
            <span class="author">Oliver A Kennedy</span> (University at Buffalo, SUNY), and
            <span class="author">Sudeepa Roy</span> (Duke University, USA)
        </span>
    </p>
    <p>
        <strong class="accepted-title">BlackMagic: Automatic Inlining of Scalar UDFs into SQL Queries with Froid</strong>
        <span class="accepted-authors">
            <span class="author">Karthik Ramachandra</span> (Microsoft Gray Systems Lab) and
            <span class="author">Kwanghyun Park</span> (Microsoft Gray Systems Lab)
        </span>
    </p>
    <p>
        <strong class="accepted-title">ProgressiveDB: Progressive Data Analytics as a Middleware</strong>
        <span class="accepted-authors">
            <span class="author">Lukas Berg</span> (TU Darmstadt),
            <span class="author">Tobias Ziegler</span> (TU Darmstadt),
            <span class="author">Carsten Binnig</span> (TU Darmstadt), and
            <span class="author">Uwe Roehm</span> (The University of Sydney)
        </span>
    </p>
    <p>
        <strong class="accepted-title">doppioDB 2.0: Hardware Techniques for Improved Integration of Machine Learning into Databases</strong>
        <span class="accepted-authors">
            <span class="author">Kaan Kara</span> (ETHZ),
            <span class="author">Zeke Wang</span> (ETHZ),
            <span class="author">Gustavo Alonso</span> (ETHZ), and
            <span class="author">Ce Zhang</span> (ETH)
        </span>
    </p>
    <p>
        <strong class="accepted-title">COVIZ: A System for Visual Formation and Exploration of Patient Cohorts</strong>
        <span class="accepted-authors">
            <span class="author">Cicero A. L. Pahins</span> (UFRGS),
            <span class="author">Behrooz Omidvar-Tehrani</span> (LIG),
            <span class="author">Sihem Amer-Yahia</span> (CNRS),
            <span class="author">Valerie Siroux</span> (Institute for Advanced Biosciences and Inserm),
            <span class="author">Jean-Louis Pepin</span> (Grenoble University Hospital ),
            <span class="author">Jean-Christian Borel</span> (AGIR-a-Dom), and
            <span class="author">Joao Luiz Dihl Comba</span> (UFRGS)
        </span>
    </p>
    <p>
        <strong class="accepted-title">PRIMAT - A Toolbox for Fast Privacy-preserving Matching</strong>
        <span class="accepted-authors">
            <span class="author">Martin Franke</span> (University of Leipzig),
            <span class="author">Ziad Sehili</span> (University of Leipzig), and
            <span class="author">Erhard Rahm</span> (University of Leipzig)
        </span>
    </p>
    <p>
        <strong class="accepted-title">NashDB: Fragmentation, Replication, and Provisioning using Economic Methods</strong>
        <span class="accepted-authors">
            <span class="author">Ryan C Marcus</span> (Brandeis University),
            <span class="author">Olga Papaemmanouil</span> (Brandeis University),
            <span class="author">Shuai Yu</span> (Brandeis University),
            <span class="author">Geoffrey Kao</span> (Brandeis University), and
            <span class="author">Chi Zhang</span> (Brandeis university)
        </span>
    </p>
    <p>
        <strong class="accepted-title">Flash in Action: Scalable Spatial Data Analysis Using Markov Logic Networks</strong>
        <span class="accepted-authors">
            <span class="author">Ibrahim Sabek</span> (University of Minnesota - Twin Cities),
            <span class="author">Mashaal Musleh</span> (University of Minnesota), and
            <span class="author">Mohamed Mokbel</span> (Qatar Computing Research Institute)
        </span>
    </p>
    <p>
        <strong class="accepted-title">I Can't Believe It's Not (Only) Software! Bionic Distributed Storage for Parquet Files</strong>
        <span class="accepted-authors">
            <span class="author">Lucas Kuhring</span> (IMDEA Software) and
            <span class="author">Zsolt Istvan</span> (IMDEA Software)
        </span>
    </p>
    <p>
        <strong class="accepted-title">VISE: Vehicle Image Search Engine with Traffic Camera</strong>
        <span class="accepted-authors">
            <span class="author">Hyewon Choi</span> (University of Toronto),
            <span class="author">Erkang Zhu</span> (University of Toronto),
            <span class="author">Arsala Bangash</span> (University of Toronto), and
            <span class="author">Renee J. Miller</span> (Northeastern University)
        </span>
    </p>
    <p>
        <strong class="accepted-title">WiClean: A System for Fixing Wikipedia Interlinks Using Revision History Patterns</strong>
        <span class="accepted-authors">
            <span class="author">Stephan Goldberg</span> (Tel Aviv University),
            <span class="author">Tova Milo</span> (Tel Aviv University),
            <span class="author">Slava Novgorodov</span> (eBay Research), and
            <span class="author">Kathy Razmadze</span> (Tel Aviv University)
        </span>
    </p>
    <p>
        <strong class="accepted-title">SparkCruise: Handsfree Computation Reuse in Spark</strong>
        <span class="accepted-authors">
            <span class="author">Abhishek Roy</span> (Microsoft),
            <span class="author">Alekh Jindal</span> (Microsoft),
            <span class="author">Hiren Patel</span> (Microsoft),
            <span class="author">Ashit Gosalia</span> (Microsoft), and
            <span class="author">Subru Krishnan</span> (Microsoft),
            <span class="author">Carlo Curino</span> (Microsoft)

    </span>
    </p>
    <p>
        <strong class="accepted-title">In-database Distributed Machine Learning: Demonstration using Teradata SQL Engine</strong>
        <span class="accepted-authors">
            <span class="author">Sandeep Singh Sandha</span> (UCLA),
            <span class="author">Wellington Cabrera</span> (Teradata Labs),
            <span class="author">Mohammed Al-Kateb</span> (Teradata Labs),
            <span class="author">Sanjay Nair</span> (Teradata), and
            <span class="author">Mani Srivastava</span> (UCLA)
        </span>
    </p>
    <p>
        <strong class="accepted-title">SHOAL: Large-scale Hierarchical Taxonomy via Graph-based Query Coalition in E-commerce</strong>
        <span class="accepted-authors">
            <span class="author">Zhao Li</span> (Alibaba Group),
            <span class="author">Xia Chen</span> (Alibaba Group),
            <span class="author">Xuming Pan</span> (Alibaba Group),
            <span class="author">Pengcheng Zou</span> (Alibaba Group),
            <span class="author">Yuchen Li</span> (Singapore Management University), and
            <span class="author">Guoxian Yu</span> (Southwest University, China)
        </span>
    </p>
    <p>
        <strong class="accepted-title">DPSAaS: Multi-Dimensional Data Sharing and Analytics as Services under Local Differential Privacy</strong>
        <span class="accepted-authors">
            <span class="author">Min Xu</span> (University of Chicago),
            <span class="author">Tianhao Wang</span> (Purdue University),
            <span class="author">Bolin Ding</span> (Alibaba Group),
            <span class="author">Jingren Zhou</span> (Alibaba Group),
            <span class="author">Cheng Hong</span> (Alibaba Group), and
            <span class="author">Zhicong Huang</span> (Alibaba Group)
        </span>
    </p>
    <p>
        <strong class="accepted-title">PriSTE: Protecting Spatiotemporal Event Privacy in Continuous Location-Based Services</strong>
        <span class="accepted-authors">
            <span class="author">Yang Cao</span> (Kyoto University),
            <span class="author">Yonghui Xiao</span> (Google),
            <span class="author">Li Xiong</span> (Emory University),
            <span class="author">Liquan Bai</span> (Emory University), and
            <span class="author">Masatoshi Yoshikawa</span> (Kyoto University)
        </span>
    </p>
    <p>
        <strong class="accepted-title">Datalignment: Ontology Schema Alignment Through Datalog Containment</strong>
        <span class="accepted-authors">
            <span class="author">Daniel Deutch</span> (Tel Aviv University),
            <span class="author">Evgeny Marants</span> (Tel Aviv University), and
            <span class="author">Yuval Moskovitch</span> (Tel Aviv University)
        </span>
    </p>
    <p>
        <strong class="accepted-title">IHCS: An Integrated Hybrid Cleaning System</strong>
        <span class="accepted-authors">
            <span class="author">Congcong Ge</span> (Zhejiang University),
            <span class="author">Yunjun Gao</span> (Zhejiang University),
            <span class="author">Xiaoye Miao</span> (Zhejiang University),
            <span class="author">Lu Chen</span> (Aalborg University),
            <span class="author">Christian S Jensen</span> (Aalborg University), and
            <span class="author">Ziyuan Zhu</span> (Zhejiang University)
        </span>
    </p>
    <p>
        <strong class="accepted-title">CAPRIO: Graph-based Integration of Indoor and Outdoor Data for Path Discovery</strong>
        <span class="accepted-authors">
            <span class="author">Constantinos Costa</span> (University of Pittsburgh),
            <span class="author">Xiaoyu Ge</span> (University of Pittsburgh), and
            <span class="author">Panos K. Chrysanthis</span> (University of Pittsburgh)
        </span>
    </p>
    <p>
        <strong class="accepted-title">HERMIT in Action: Succinct Secondary Indexing Mechanism via Correlation Exploration</strong>
        <span class="accepted-authors">
            <span class="author">Yingjun Wu</span> (IBM Almaden Research Center),
            <span class="author">Jia Yu</span> (Arizona State University),
            <span class="author">Yuanyuan Tian</span> (IBM Almaden),
            <span class="author">Richard Sidle</span> (IBM), and
            <span class="author">Ronald Barber</span> (IBM Research)
        </span>
    </p>
    <p>
        <strong class="accepted-title">DISPERS: Securing Highly Distributed Queries on Personal Data Management Systems</strong>
        <span class="accepted-authors">
            <span class="author">Julien Loudet</span> (INRIA),
            <span class="author">Iulian Sandu Popa</span> (INRIA & David Lab., UVSQ), and
            <span class="author">Luc Bouganim</span> (INRIA)
        </span>
    </p>
    <p>
        <strong class="accepted-title">Stateful Functions as a Service in Action</strong>
        <span class="accepted-authors">
            <span class="author">Adil Akhter</span> (ING),
            <span class="author">Marios Fragkoulis</span> (TU Delft), and
            <span class="author">Asterios Katsifodimos</span> (TU Delft)
        </span>
    </p>
    <p>
        <strong class="accepted-title">Demonstration of Krypton: Optimized CNN Inference for Occlusion-based Deep CNN Explanations</strong>
        <span class="accepted-authors">
            <span class="author">Allen Ordookhanians</span> (University of California, San Diego),
            <span class="author">Xin Li</span> (University of California, San Diego),
            <span class="author">Supun Nakandala</span> (University of California, San Diego), and
            <span class="author">Arun Kumar</span> (University of California, San Diego)
        </span>
    </p>
    <p>
        <strong class="accepted-title">LensXPlain: Visualizing and Explaining Contributing Subsets for Aggregate Query Answers</strong>
        <span class="accepted-authors">
            <span class="author">Zhengjie Miao</span> (Duke University),
            <span class="author">Andrew Lee</span> (Duke University), and
            <span class="author">Sudeepa Roy</span> (Duke University, USA)
        </span>
    </p>
    <p>
        <strong class="accepted-title">Juneau: Data Lake Management for Jupyter</strong>
        <span class="accepted-authors">
            <span class="author">Yi Zhang</span> (University of Pennsylvania) and
            <span class="author">Zack Ives</span> (University of Pennsylvania)
        </span>
    </p>
    <p>
        <strong class="accepted-title">ApproxML: Efficient Approximate Ad-Hoc ML Models Through Materialization and Reuse</strong>
        <span class="accepted-authors">
            <span class="author">Sona Hasani</span> (University of Texas at Arlington),
            <span class="author">Faezeh Ghaderi</span> (University of Texas at Arlington),
            <span class="author">Shohedul Hasan</span> (UTA),
            <span class="author">Saravanan Thirumuruganathan</span> (QCRI),
            <span class="author">Abolfazl Asudeh</span> (University of Michigan),
            <span class="author">Nick Koudas</span> (University of Toronto), and
            <span class="author">Gautam Das</span> (U. of Texas Arlington)
        </span>
    </p>
    <p>
        <strong class="accepted-title">Flare & Lantern: Efficiently Swapping Horses Midstream</strong>
        <span class="accepted-authors">
            <span class="author">Gregory Essertel</span> (Purdue University),
            <span class="author">Ruby Tahboub</span> (Purdue University),
            <span class="author">Fei Wang</span> (Purdue University),
            <span class="author">James Decker</span> (Purdue University), and
            <span class="author">Tiark Rompf</span> (Purdue University)
        </span>
    </p>
    <p>
        <strong class="accepted-title">Trinity: An Extensible Synthesis Framework for Data Science</strong>
        <span class="accepted-authors">
            <span class="author">Ruben Martins</span> (CMU),
            <span class="author">Jia Chen</span> (UT Austin),
            <span class="author">Yanju Chen</span> (UCSB),
            <span class="author">Yu Feng</span> (UCSB), and
            <span class="author">Isil Dillig</span> (UT Austin)
        </span>
    </p>
    <p>
        <strong class="accepted-title">PSynDB: Accurate and Accessible Private Data Generation</strong>
        <span class="accepted-authors">
            <span class="author">Zhiqi Huang</span> (University of Massachusetts, Amherst),
            <span class="author">Ryan McKenna</span> (University of Massachusetts, Amherst),
            <span class="author">Gerome Miklau</span> (University of Massachusetts, Amherst),
            <span class="author">Michael Hay</span> (Colgate University), and
            <span class="author">Ashwin Machanavajjhala</span> (Duke)
        </span>
    </p>
    <p>
        <strong class="accepted-title">FishStore: Fast Ingestion and Indexing of Raw Data</strong>
        <span class="accepted-authors">
            <span class="author">Badrish Chandramouli</span> (Microsoft Research),
            <span class="author">Dong Xie</span> (University of Utah),
            <span class="author">Yinan Li</span> (Microsoft Research), and
            <span class="author">Donald Kossmann</span> (Microsoft Research)
        </span>
    </p>
    <p>
        <strong class="accepted-title">Spade: A Modular Framework for Analytical Exploration of RDF Graphs</strong>
        <span class="accepted-authors">
            <span class="author">Yanlei Diao</span> (Ecole Polytechnique),
            <span class="author">Pawel Guzewicz</span> (INRIA),
            <span class="author">Ioana Manolescu</span> (INRIA Saclay, France), and
            <span class="author">Mirjana Mazuran</span> (INRIA Saclay)
        </span>
    </p>
    <p>
        <strong class="accepted-title">Democratizing Data Science with AIDA: Advanced In-database Interactive Analytics with Visualization Support</strong>
        <span class="accepted-authors">
            <span class="author">Joseph Vinish D'silva</span> (McGill University),
            <span class="author">Florestan De Moor</span> (McGill University), and
            <span class="author">Bettina Kemme</span> (McGill University)
        </span>
    </p>
    <p>
        <strong class="accepted-title">UDAO: A Next-Generation Unified Data Analytics Optimizer</strong>
        <span class="accepted-authors">
            <span class="author">Khaled Zaouk</span> (Ecole Polytechnique),
            <span class="author">Fei Song</span> (Ecole Polytechnique),
            <span class="author">Chenghao Lyu</span> (University of Massachusetts, Amherst),
            <span class="author">Yanlei Diao</span> (University of Massachusetts, Amherst), and
            <span class="author">Prashant Shenoy</span> (University of Massachusetts, Amherst)
        </span>
    </p>
    <p>
        <strong class="accepted-title">AggChecker: A Fact-Checking System for Text Summaries of Relational Data Sets</strong>
        <span class="accepted-authors">
            <span class="author">Saehan Jo</span> (Cornell University),
            <span class="author">Immanuel Trummer</span> (Cornell),
            <span class="author">Weicheng Yu</span> (Cornell University),
            <span class="author">Xuezhi Wang</span> (Google),
            <span class="author">Cong Yu</span> (Google),
            <span class="author">Daniel Liu</span> (Cornell University), and
            <span class="author">Niyati Mehta</span> (Cornell University)
        </span>
    </p>
    <p>
        <strong class="accepted-title">GRANO: Interactive Graph-based Root Cause Analysis for Cloud-Native Distributed Data Platform</strong>
        <span class="accepted-authors">
            <span class="author">Hanzhang Wang</span> (eBay),
            <span class="author">Phuong Nguyen</span> (eBay),
            <span class="author">Jun Li</span> (eBay),
            <span class="author">Selcuk Kopru</span> (eBay),
            <span class="author">Gene Zhang</span> (eBay Inc),
            <span class="author">Sanjeev Katariya</span> (eBay), and
            <span class="author">Sami Ben-Romdhane</span> (eBay Inc)
        </span>
    </p>
    <p>
        <strong class="accepted-title">Dietcoin: Hardening Bitcoin Transaction Verification Process For Mobile Devices</strong>
        <span class="accepted-authors">
            <span class="author">Davide Frey</span> (University of Rennes),
            <span class="author">Marc X. Makkes</span> (Vrije Universiteit),
            <span class="author">Pierre-Louis Roman</span> (Univ Rennes, Inria, CNRS, IRISA),
            <span class="author">Francois Taiani</span> (Univ Rennes, CNRS, Inria, IRISA), and
            <span class="author">Spyros Voulgaris</span> (Athens University of Economics and Business)
        </span>
    </p>
    <p>
        <strong class="accepted-title">Raptor: Large Scale Analysis of Big Raster and Vector Data</strong>
        <span class="accepted-authors">
            <span class="author">Samriddhi Singla</span> (University of California, Riverside),
            <span class="author">Ahmed Eldawy</span> (University of California, Riverside),
            <span class="author">Rami Alghamdi</span> (University of Minnesota, Twin Cities), and
            <span class="author">Mohamed Mokbel</span> (Qatar Computing Research Institute)
        </span>
    </p>
    <p>
        <strong class="accepted-title">Data Civilizer 2.0: A Holistic Framework for Data Preparation and Analytics</strong>
        <span class="accepted-authors">
            <span class="author">El Kindi Rezig</span> (MIT),
            <span class="author">Lei Cao</span> (MIT),
            <span class="author">Michael Stonebraker</span> (MIT),
            <span class="author">Giovanni Simonini</span> (MIT),
            <span class="author">Wenbo Tao</span> (MIT),
            <span class="author">Samuel Madden</span> (MIT),
            <span class="author">Mourad Ouzzani</span> (Qatar Computing Research Institute, HBKU),
            <span class="author">Nan Tang</span> (Qatar Computing Research Institute, HBKU), and
            <span class="author">Ahmed Elmagarmid</span> (QCRI)
        </span>
    </p>
    <p>
        <strong class="accepted-title">Tuplex: Robust, efficient analytics when Python rules</strong>
        <span class="accepted-authors">
            <span class="author">Leonhard Spiegelberg</span> (Brown University) and
            <span class="author">Tim Kraska</span> (MIT)
        </span>
    </p>
    <p>
        <strong class="accepted-title">Ease.ml/ci and Ease.ml/meter in Action: Towards Data Management for Statistical Generalization</strong>
        <span class="accepted-authors">
            <span class="author">Cedric Renggli</span> (ETH Zurich),
            <span class="author">Frances Ann Hubis</span> (ETH Zurich),
            <span class="author">Bojan Karlas</span> (ETH Zurich),
            <span class="author">Kevin Schawinski</span> (Modulos AG),
            <span class="author">Wentao Wu</span> (Microsoft Research), and
            <span class="author">Ce Zhang</span> (ETH)
        </span>
    </p>
    <p>
        <strong class="accepted-title">PivotE: Revealing and Visualizing the Underlying EntityStructures for Exploration</strong>
        <span class="accepted-authors">
            <span class="author">Han Xueran</span> (Renmin Unversity of China),
            <span class="author">Jun Chen</span> (Renmin University of China), and
            <span class="author">Yueguo Chen</span> (Renmin University of China)
        </span>
    </p>
</section>
<?php
define('PageTitle', "Program Schedule - Demonstrations");
define('PageDescription', "List of accepted demonstrations.");
?>
<script type="application/ld+json">
{
  "@context": "http://schema.org",
  "@type": "WebPage",
  "name": "Program Schedule - Demonstrations",
  "url": "https://vldb.org/2019/?program-schedule-demonstrations",
  "description": "List of accepted demonstrations.",
  "breadcrumb": {
    "@type": "BreadcrumbList",
    "itemListElement": [{
      "@type": "ListItem",
      "position": 1,
      "item": {
        "@id": "https://vldb.org/",
        "name": "VLDB"
      }
    },{
      "@type": "ListItem",
      "position": 2,
      "item": {
        "@id": "https://vldb.org/conference.html",
        "name": "Conferences"
      }
    },{
      "@type": "ListItem",
      "position": 3,
      "item": {
        "@id": "https://vldb.org/2019/",
        "name": "2019"
      }
    },{
      "@type": "ListItem",
      "position": 4,
      "item": {
        "@id": "https://vldb.org/2019/?program",
        "name": "Conference Program"
      }
    },{
      "@type": "ListItem",
      "position": 5,
      "item": {
        "@id": "https://vldb.org/2019/?program-schedule-demonstrations",
        "name": "Demonstrations"
      }
    }]
  }
}


</script>